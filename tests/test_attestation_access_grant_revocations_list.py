"""Tests for the global read-only attestation access-grant-revocation search.

Covers ``GET /v1/attestation-access-grant-revocations``: the exact
``items``/``count``/``next_cursor`` member order in compact UTF-8 JSON
(UTF-8, non-ASCII unescaped) terminated by one newline, each item reusing
the single-revocation public view (no raw signature, private key, payload,
content, evidence bytes, authentication header, or ordering column), stable
creation ordering (``created_at`` with the persistent insertion-order
...
validation (1..100, default 50), the opaque HMAC-signed ``agr1`` cursor
...
absence of any authentication requirement, the 405 rejection of
...
def _attest(client, target_id, *, seed, signer_actor_id):
...
def _grant(client, attestation_id, grantee_actor_id, *, actor, seed):
    body = json.dumps(
        {
...
def _revoke(client, grant_id, reason, *, actor="org-1", seed=SEED_A):
    body = json.dumps({"grant_id": grant_id, "reason": reason}).encode(
        "utf-8"
    )
    headers = {
        "Content-Type": "application/json",
        **_signed_headers(
            "POST", SEARCH_PATH, body, actor=actor, seed=seed
        ),
    }
    resp = client.post(SEARCH_PATH, content=body, headers=headers)
    assert resp.status_code == 201, resp.text
    return resp.json()


def _world(client):
    """Two signers, four grants, four interleaved grant revocations.

    Creation order:
      r1: grant g1 (org-1's proof -> org-2), revoker org-1, REASON_A
      r2: grant g1 again,                    revoker org-1, REASON_B
      r3: grant g3 (org-2's proof -> org-1), revoker org-2, REASON_A
      r4: grant g4 (org-2's proof -> org-3), revoker org-2, REASON_C

    g2 (org-1's proof -> org-3) is granted but never revoked, so filtering
    by it is an unknown-value empty collection even though the grant exists.
    """
    ...
    att_one = _attest(
        client,
        _create_claim(client, _create_content(client)["id"])["id"],
        seed=SEED_A,
    ...
    att_two = _attest(
        ...
        _create_claim(
            client,
            _create_content(client, actor_id="org-2", digest=DIGEST_B)["id"],
            actor_id="org-2",
        )["id"],
        seed=SEED_B,
    ...
    g1 = _grant(client, att_one["id"], "org-2", actor="org-1", seed=SEED_A)
    g2 = _grant(client, att_one["id"], "org-3", actor="org-1", seed=SEED_A)
    g3 = _grant(client, att_two["id"], "org-1", actor="org-2", seed=SEED_B)
    g4 = _grant(client, att_two["id"], "org-3", actor="org-2", seed=SEED_B)

    r1 = _revoke(client, g1["id"], REASON_A)
    r2 = _revoke(client, g1["id"], REASON_B)
    r3 = _revoke(client, g3["id"], REASON_A, actor="org-2", seed=SEED_B)
    r4 = _revoke(client, g4["id"], REASON_C, actor="org-2", seed=SEED_B)
    return {
        "att_one": att_one,
        "att_two": att_two,
        "g1": g1,
...
def test_search_requires_no_authentication_headers(client):
    grants, _ = _world(client)
    resp = client.get(
        SEARCH_PATH, params={"grant_id": grants["g1"]["id"]}
    )
    assert resp.status_code == 200, resp.text
...
def test_non_ascii_reason_is_utf8_and_never_escaped(client):
    grants, _ = _world(client)
    created = _revoke(client, grants["g2"]["id"], REASON_UNICODE)
    resp = _list(client, grant_id=grants["g2"]["id"])
    assert resp.status_code == 200
    assert resp.json()["items"] == [created]
    raw = resp.content
...
def test_items_carry_exactly_the_revocation_public_view(client):
    ...
    # No signature, key, payload, content/evidence byte, auth material, or
    # ordering column is echoed.
...
def test_ordering_is_identical_across_an_app_restart(tmp_db_url, file_client):
    ...
    _, revocations = _world(file_client)
...
def test_grant_id_filter_is_an_exact_match(client):
    grants, revocations = _world(client)
    body = _list(client, grant_id=grants["g1"]["id"]).json()
    assert body["count"] == 2
    assert body["next_cursor"] is None
    assert [item["id"] for item in body["items"]] == [
        revocations[0]["id"],
    ...
    # The granted-but-never-revoked g2 is an empty collection, not a 404.
    assert _list(client, grant_id=grants["g2"]["id"]).json() == {
        "items": [],
        "count": 0,
        "next_cursor": None,
    }
...
def test_filters_are_case_and_whitespace_sensitive(client):
    grants, _ = _world(client)
    for params in (
        {"grant_id": grants["g1"]["id"].upper()},
        {"grant_id": f" {grants['g1']['id']}"},
        {"grant_id": f"{grants['g1']['id']} "},
        {"revoker_actor_id": "Org-1"},
        {"revoker_actor_id": " org-1"},
        {"revoker_actor_id": "org-1 "},
...
def _pinned_world(client, db_session, instants):
    _, revocations = _world(client)
...
def test_from_bound_is_inclusive(client, db_session):
    revocations = _pinned_world(client, db_session, _JAN_DAYS)
...
def test_equal_from_and_to_selects_exactly_that_instant(client, db_session):
    ...
    revocations = _pinned_world(client, db_session, instants)
    body = _list(
...
def test_default_limit_is_fifty(client):
    ...
    grant = _grant(client, att["id"], "org-2", actor="org-1", seed=SEED_A)
    # Distinct reasons on one grant are independent immutable records.
    for index in range(51):
        _revoke(client, grant["id"], f"reason-{index:03d}")
    resp = _list(client)
    assert resp.status_code == 200
    body = resp.json()
...
def test_repeated_and_undeclared_parameters_are_422(client):
    ...
    assert (
        client.get(
            SEARCH_PATH,
            params=[("grant_id", "aag_a"), ("grant_id", "aag_b")],
        ).status_code
        == 422
    )
    assert (
...
def test_pages_resume_without_duplication_or_omission(client):
    ...
    assert first["next_cursor"].startswith("agr1.")
...
def test_count_covers_every_filtered_page(client):
    ...
    all_items, _pages, count = _walk_pages(
        client, grant_id=grants["g1"]["id"], limit="1"
    )
    assert count == 2
    assert len(all_items) == 2
    assert {item["grant_id"] for item in all_items} == {grants["g1"]["id"]}
...
def test_cursor_binds_every_effective_filter_and_limit(client):
    ...
    switched = _list(
        client, grant_id=grants["g3"]["id"], limit="1", cursor=g1_cursor
    )
...
def test_blank_malformed_and_tampered_cursors_are_422(client):
    ...
    for bad in ("", "   ", "not-a-cursor", "agr1", "agr1.abc", tampered):
...
def test_foreign_family_cursor_is_422(client, app):
    ...
    # The sibling ar1 attestation-revocation family is also foreign here.
    sibling = pagination.encode_typed_cursor(
    ...
    assert _list(client, cursor=sibling).status_code == 422
...
def test_cursor_with_wrong_claim_set_is_422(client, app):
    ...
    sig = base64.urlsafe_b64encode(
        hmac.new(
            app.state.attestation_access_grant_revocations_cursor_secret,
            f"agr1.{payload}".encode(),
            hashlib.sha256,
        ).digest()
    ).rstrip(b"=").decode()
    resp = _list(client, cursor=f"agr1.{payload}.{sig}")
    assert resp.status_code == 422
    assert resp.json()["error"]["code"] == "validation_error"
...
def test_post_remains_the_revocation_creation_route(client):
    # The new read entry does not shadow or alter signed revocation creation.
    create_actor(client, actor_id="org-1")
    create_actor(client, actor_id="org-2", name="Other", type="organization")
    att = _attest(
        client,
    ...
    grant = _grant(client, att["id"], "org-2", actor="org-1", seed=SEED_A)
    first = _revoke(client, grant["id"], REASON_A)
    assert first["reason"] == REASON_A
    # An idempotent retry for the same triple returns the original record
    # with 200; the GET search still exposes exactly one row for the grant.
    body = json.dumps(
    ...
    headers = {
        "Content-Type": "application/json",
        **_signed_headers(
            "POST", SEARCH_PATH, body, actor="org-1", seed=SEED_A
        ),
    }
    repeat_resp = client.post(SEARCH_PATH, content=body, headers=headers)
    assert repeat_resp.status_code == 200
    assert repeat_resp.json() == first
    searched = _list(client, grant_id=grant["id"]).json()
...
def test_unicode_reason_is_searchable_verbatim(client):
    grants, _ = _world(client)
    created = _revoke(client, grants["g2"]["id"], REASON_UNICODE)
    # Exact, case- and whitespace-sensitive verbatim match.
    body = _list(client, reason=REASON_UNICODE).json()
    assert body["count"] == 1
    assert body["items"] == [created]
    # A normalized/blank variant never matches the stored value.
    assert _list(
        client, reason=f" {REASON_UNICODE}"
    ).json()["items"] == []
...
def test_queries_and_failures_write_nothing(client, db_session):
    ...
    for method in (client.put, client.patch, client.delete):
        assert method(SEARCH_PATH).status_code == 405
