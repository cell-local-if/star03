"""Tests for the read-only signed audit exchange receipt retrieval.

Covers GET /v1/audit-exchanges: receipts follow stable creation order and
each item is exactly the single-receipt public view (``id``,
``signature_version``, ``signer_subject``, ``public_key``,
...
whitespace-sensitive exact matches combining as logical AND; an unknown
value is an empty collection, never an error. ``count`` is the filtered
total covering every page; ``limit`` is a pure decimal integer in
1..100 defaulting to 50; the opaque HMAC cursor (its own ``ax1``
family) binds every effective filter and the limit, so pages concatenate
without gaps or duplicates and the final cursor is null. A non-empty GET
body, a blank/illegal/repeated/undeclared parameter, and a
blank/malformed/tampered/foreign-family/query-mismatching cursor are all
422 validation_error; non-GET methods (other than the import POST) are
405 method_not_allowed. Queries and failures write no receipt and no
audit row.
"""
...
def _import(client, n, *, subject="remote-system", seed=SEED_A):
    package = _package(_events(n))
...
def test_filter_by_signature_version(client):
    _three_receipts(client)
    assert _list(client, signature_version=SIGNATURE_VERSION).json()["count"] == 3
...
def test_filtered_pagination(client):
    _three_receipts(client)
    items, _, count = _walk_pages(client, signer_subject="remote-system", limit=1)
    assert count == 2
...
def test_cursor_past_end_returns_empty_page_with_total_count(client, app):
    ...
    token = pagination.encode_typed_cursor(
        app.state.audit_exchange_imports_cursor_secret,
...
def test_cursor_from_other_families_is_rejected(client):
    _three_receipts(client)
    recon_cursor = pagination.encode_typed_cursor(
        secrets.token_bytes(32),
    ...
    for token in (recon_cursor, checkpoint_cursor):
        _assert_validation_error(_list(client, limit=1, cursor=token))
...
def test_listing_is_stable_across_restart(tmp_db_url):
    from fastapi.testclient import TestClient
    ...
    app_one = create_app(Settings(database_url=tmp_db_url))
    with TestClient(app_one) as client_one:
        r1, r2, r3 = _three_receipts(client_one)
        expected = [r1["id"], r2["id"], r3["id"]]
    app_two = create_app(Settings(database_url=tmp_db_url))
    with TestClient(app_two) as client_two:
