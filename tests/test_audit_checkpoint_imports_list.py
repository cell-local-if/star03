...
matches while ``event_count`` is a non-negative decimal integer; the three
filters combine as logical AND (absent means unfiltered). ``limit`` is
...
def _events_for(label: str, count: int) -> list:
    """A deterministic self-contained event sequence of ``count`` events."""
    return [
        {
            "event_type": f"offline.{label}",
            "resource_id": f"res-{label}-{index}",
            "created_at": f"2026-01-02T03:04:{index:02d}Z",
        }
...
def _import(client, label: str, count: int = 2) -> dict:
    ...
    resp = client.post(URL, json=_offline_request(_events_for(label, count)))
    assert resp.status_code == 201, resp.text
    return resp.json()


def _setup_grouped(client):
    """Three two-event receipts and two three-event receipts, in order."""
...
def test_item_is_the_public_receipt_and_matches_the_single_read(client):
    ...
    # The event array and raw materials stay absent from the list view.
...
def test_listing_after_retry_keeps_the_original_position(client):
    ...
    retry = client.post(URL, json=_offline_request(_events_for("once", 2)))
...
def test_filters_combine_as_logical_and(client):
    ...
    # A count paired with another receipt's digest satisfies nothing: the
...
def test_cursor_bound_to_every_effective_query_parameter(client):
    ...
    assert (
        _list(
            client,
            events_digest_hex=receipts[1]["events_digest_hex"],
            limit=2,
            cursor=digested,
        ).status_code
        == 422
    ...
    counted = _list(client, event_count=2, limit=1).json()["next_cursor"]
    assert _list(client, limit=1, cursor=counted).status_code == 422
    assert (
        _list(client, event_count=3, limit=1, cursor=counted).status_code
        == 422
    )
...
def test_illegal_event_count_values_are_validation_errors(client):
    _setup_grouped(client)
    for value in ("-1", "1.5", "abc", "8.0", "  2", "2 ", "+2", "1e3"):
