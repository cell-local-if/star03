"""Tests for the read-only checkpoint-import receipt search endpoint.

Covers GET /v1/audit-events/checkpoint-imports: the response shape carries
exactly ``{"items", "count", "next_cursor"}`` with each item exposing only
the existing single-receipt fields ``id``/``checkpoint_version``/
``events_digest_hex``/``event_count``/``received_at`` (timezone-aware UTC),
receipts follow stable creation order, and ``checkpoint_version``/
``events_digest_hex`` are non-empty, case- and whitespace-sensitive exact
matches while ``event_count`` is a non-negative decimal integer match; the
three combine as logical AND (absent means unfiltered). ``limit`` is
1..100 defaulting to 50; the opaque HMAC cursor binds every effective
filter and the limit so pages concatenate without gaps or duplicates and
the final cursor is null. Blank/illegal/repeated/undeclared parameters and
blank/malformed/tampered/foreign-family/query-mismatching cursors are 422
validation_error; no match is an empty collection. Neither queries nor
failures write any receipt or audit row. All fixtures are deterministic
and offline.
"""
...
def _import(client, label: str, count: int = 2) -> dict:
    ...
    resp = client.post(URL, json=_offline_request(_events(label, count)))
    assert resp.status_code == 201, resp.text
    return resp.json()


def _setup_grouped(client):
    """Three receipts with event_count 2 and two with event_count 3.

    Returns the receipts in creation order.
    """
...
def test_item_is_the_public_receipt_and_matches_the_single_read(client):
    ...
    # The event array and the checkpoint envelope stay absent from the list.
...
def test_listing_after_retry_keeps_the_original_position(client):
    ...
    # Re-presenting the same checkpoint is the idempotent 200 retry, not a
    # new receipt and not a new position in the listing.
    retry = client.post(URL, json=_offline_request(_events("once", 2)))
...
def test_checkpoint_version_exact_match(client):
    ...
    body = _list(
        client, checkpoint_version="provenance-audit-checkpoint-v2"
    ).json()
    assert body == {"items": [], "count": 0, "next_cursor": None}


def test_events_digest_hex_exact_match(client):
    receipts = _setup_grouped(client)
    target = receipts[1]
    body = _list(
        client, events_digest_hex=target["events_digest_hex"]
    ).json()
    assert body["count"] == 1
    assert body["items"][0] == target


def test_event_count_exact_match(client):
    _setup_grouped(client)
    body = _list(client, event_count=2).json()
    assert body["count"] == 3
    assert {i["event_count"] for i in body["items"]} == {2}

    body = _list(client, event_count=3).json()
    assert body["count"] == 2
    assert {i["event_count"] for i in body["items"]} == {3}

    # No receipt commits to an empty sequence here.
    body = _list(client, event_count=0).json()
    assert body == {"items": [], "count": 0, "next_cursor": None}


def test_event_count_zero_matches_empty_sequence_receipts(client):
...
def test_filters_combine_as_logical_and(client):
    ...
    body = _list(
        ...
        events_digest_hex=target["events_digest_hex"],
        event_count=2,
    ).json()
    assert [i["id"] for i in body["items"]] == [target["id"]]
    assert body["count"] == 1

    # A digest paired with another receipt's count satisfies nothing: the
    # filters are ANDed, not coerced.
    body = _list(
        client,
        events_digest_hex=target["events_digest_hex"],
        event_count=3,
...
def test_exact_match_is_case_and_whitespace_sensitive(client):
    receipts = _setup_grouped(client)
    digest = receipts[0]["events_digest_hex"]
    for params in (
        {"checkpoint_version": CHECKPOINT_VERSION.upper()},
        {"checkpoint_version": f" {CHECKPOINT_VERSION}"},
        {"checkpoint_version": f"{CHECKPOINT_VERSION} "},
        {"events_digest_hex": digest.upper()},
        {"events_digest_hex": f" {digest}"},
        {"events_digest_hex": f"{digest} "},
    ):
...
def test_no_matching_receipts_is_empty_collection(client):
    _setup_grouped(client)
    body = _list(client, events_digest_hex="0" * 64).json()
    assert body == {"items": [], "count": 0, "next_cursor": None}
...
def test_count_is_filtered_total_on_every_page(client):
    _setup_grouped(client)
    all_items, pages, count = _walk_pages(client, event_count=2, limit=2)
    assert count == 3
    assert [len(page) for page in pages] == [2, 1]
    assert {i["event_count"] for i in all_items} == {2}
...
def test_cursor_past_end_returns_empty_page_with_total_count(client, app):
    ...
    token = pagination.encode_typed_cursor(
        app.state.checkpoint_imports_cursor_secret,
...
def test_cursor_signed_with_old_format_marker_is_rejected(client, app):
    ...
    sig = base64.urlsafe_b64encode(
        hmac_mod.new(
            app.state.checkpoint_imports_cursor_secret,
            f"ci0.{payload}".encode(),
            hashlib.sha256,
        ).digest()
    ).rstrip(b"=").decode()
    resp = _list(client, cursor=f"ci0.{payload}.{sig}")
...
def test_foreign_family_cursor_signed_with_this_secret_is_rejected(client, app):
    # Family separation is by format marker, not by secret: an exchange-
    # imports cursor minted under this endpoint's own secret is still
    # rejected.
    _setup_grouped(client)
    foreign = pagination.encode_typed_cursor(
        app.state.checkpoint_imports_cursor_secret,
...
def test_checkpoint_imports_cursor_is_rejected_by_other_endpoints(client):
    _setup_grouped(client)
    ...
    # As does the exchange-imports receipt search.
    resp = client.get(
...
def test_cursor_bound_to_every_effective_query_parameter(client):
    ...
    digested = _list(client, events_digest_hex=digest, limit=2).json()[
        "next_cursor"
    ]
    assert _list(client, limit=2, cursor=digested).status_code == 422

    counted = _list(client, event_count=2, limit=2).json()["next_cursor"]
    assert _list(client, limit=2, cursor=counted).status_code == 422
    assert (
        _list(client, event_count=3, limit=2, cursor=counted).status_code
        == 422
    )
...
def test_cursor_secret_rotation_invalidates_outstanding_cursors(
    ...
    cursor = _list(client, limit=1).json()["next_cursor"]
    client.app.state.checkpoint_imports_cursor_secret = secrets.token_bytes(32)
...
def test_blank_filters_are_validation_errors(client):
    _setup_grouped(client)
    for field in ("checkpoint_version", "events_digest_hex", "event_count"):
        for value in ("", "   ", "\t"):
...
def test_illegal_event_count_values_are_validation_errors(client):
    _setup_grouped(client)
    for value in ("-1", "+1", "1.5", "2.0", "abc", "  2", "2 ", "1e3", "0x2"):
        resp = _list(client, event_count=value)
...
def test_repeated_parameters_are_validation_errors(client):
    ...
    for suffix in (
        ...
        "events_digest_hex=a&events_digest_hex=b",
...
def test_undeclared_parameters_are_validation_errors(client):
    ...
    for suffix in (
        "import_id=aci_x",
        "checkpoint_versions=provenance-audit-checkpoint-v1",
        "digest_hex=" + "0" * 64,
        "event_counts=2",
        "limit=1&offset=2",
        "CURSOR=x",
...
def test_queries_and_failures_write_no_rows_or_audit_events(client, db_session):
    ...
    _list(client, events_digest_hex="0" * 64)
    _list(client, event_count=2)
    _list(client, event_count=0)
    ...
    _list(client, event_count=2, cursor="garbage")
...
def test_existing_post_and_single_get_remain_unchanged(client):
    ...
    missing = client.get(f"{URL}/aci_{'0' * 64}")
...
def test_checkpoint_and_verification_routes_remain_unchanged(client):
    _import(client, "served")
    # The read-only checkpoint over local audit state still serves, and the
    # stateless verification route still verifies a consistent body.
    ...
    verification = client.post(
        ...
        json={"checkpoint": checkpoint, "events": events},
    )
