...
- `GET /v1/contents/{content_id}/export` — read-only, transferable provenance-evidence snapshot of one content for third-party verifiers. The success body is exactly `{"content", "claims"}`: `content` is the existing full public content view, and `claims` lists the claims that directly assert this exact content in their stable creation order — no lineage traversal to related contents, and no other contents' claims or bundles. Each claim item is the existing full public claim view plus `evidence_bundles`, the claim's bundles in their stable creation order, each the existing evidence-bundle public view. A content without claims yields `"claims": []`; a claim without bundles yields `"evidence_bundles": []`. No raw content, claim payloads, or evidence bytes appear — only the existing public fields, identifiers, and UTC timestamps. The route accepts no query parameters: any parameter is `422 validation_error`; an unknown content id is `404 content_not_found`. Reads, empty results, and failures write no resource or audit rows.
