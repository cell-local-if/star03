...
## Read-only audit-event search

Compliance reviewers can search the existing append-only audit trail without changing any resource or audit data.

- `GET /v1/audit-events?event_type=...&resource_id=...&from=...&to=...&limit=...&cursor=...` — read-only listing of the stored audit events in their stable creation order (`created_at`, with the monotonic insertion sequence as tiebreaker). Returns `{"items", "count", "next_cursor"}` where each item carries exactly `{event_type, resource_id, created_at}` with a timezone-aware UTC `created_at`, and `count` is the filtered total independent of the page. Optional `event_type` and `resource_id` are non-empty, case/whitespace-sensitive exact matches that combine as logical AND; `from` and `to` are optional RFC 3339 UTC timestamps (an explicit `Z` or zero offset, optional fractional seconds) that bound `created_at` inclusively on both ends, and `from` must not be later than `to`. `limit` is an integer from 1 to 100 defaulting to 50; `cursor` is an opaque HMAC-signed server token that binds to every effective filter (carried as its raw, un-normalized value) and the effective limit, so pages resume without duplication or omission and the final page carries `next_cursor: null`. Blank/illegal/repeated/undeclared parameters, a malformed or non-UTC timestamp, an inverted `from`/`to` range, and an empty, malformed, tampered, foreign-family, or query-mismatching `cursor` are all `422 validation_error` — nothing is silently defaulted or normalized. No matching events yields `{"items": [], "count": 0, "next_cursor": null}`. The query — successful or failed — writes no resource and no audit event.
