...
- `POST /v1/evidence-bundle-imports` — atomically import a batch for an external forensics system. The body is exactly `{"items": [...]}` with 1 to 100 entries, each validated by the same rules as `POST /v1/evidence-bundles` (including `extra=forbid`, so a byte-bearing `data`/`evidence` field on an item or an undeclared top-level field is `422 validation_error`). The in-batch identity is `(claim_id, evidence_type, digest_algorithm, digest_hex)`: a repeated identity keeps its first occurrence — later repeats never appear in the response, even with differing `media_type`/`metadata` — and an identity already stored returns its original public view. Every referenced claim must exist before any write, so one unknown claim is the existing `404 claim_not_found` and the whole batch creates neither bundles nor audit events; every item and reference is validated structurally first. The success body is `{"items", "count"}`: `items` are the full public bundle views in each unique identity's first-appearance order and `count` is the unique-identity count. A batch that creates at least one new bundle returns `201`; a batch whose identities all already exist returns `200` and writes nothing. All new bundles and their `evidence_bundle.created` audit events commit in a single transaction.
