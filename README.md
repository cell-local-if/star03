...
## Reviewer trust evaluations

- `GET /v1/trust-evaluations` — a read-only, on-demand evaluation of one claim or evidence bundle for reviewers. The query requires exactly one non-blank `target_type` (`claim` or `evidence_bundle`) and a non-blank `target_id`; `min_signers` is an optional strictly-formatted decimal integer from 1 to 100 defaulting to 1. The target is confirmed to exist first (after parameters validate): a missing target is `404 claim_not_found` / `404 evidence_bundle_not_found` matched by the declared `target_type`. The response is `{"target_type", "target_id", "min_signers", "qualified_signer_count", "decision"}`: `qualified_signer_count` counts only the target's `verified=true` attestations, deduplicated by `signer_actor_id` so multiple attestations from one independent signing subject (even with different keys) count once. `decision` is `trusted` when the distinct count reaches `min_signers` and `untrusted` otherwise. The result is computed at request time: no resource is created and no audit event is written. Blank/whitespace or missing `target_type`/`target_id`, an unknown `target_type`, a repeated or unknown query parameter, and a `min_signers` that is blank, non-integer (`1.5`, `8.0`), out of range, or carries a sign (`+1`) or surrounding whitespace are all `422 validation_error` — nothing is silently defaulted or normalized, and parameters validate before the target-existence check.
