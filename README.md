...
- `GET /v1/contents/{content_id}/evidence-bundles` — read-only listing for auditors of the evidence bundles already attached to that content's own claims, as `{"items", "count", "next_cursor"}`; each item is the existing bundle's public fields. It follows only the bundle→claim association: lineage-reachable contents and raw evidence bytes are never included. Optional `evidence_type` and `media_type` are non-empty **exact-match** filters (case- and whitespace-sensitive) that combine conjunctively; absent means unfiltered. Results are in the bundles' stable creation order. Paging uses `limit` (an integer from 1 to 100, default 50) and an opaque server `cursor`; a continuation cursor binds to the path content and every effective filter/limit, so resuming must repeat the same effective parameters and neither skips nor repeats items. `count` is the total after filtering (independent of the page) and `next_cursor` is `null` on the final page (including an empty result). An unknown content id remains `404 content_not_found`; blank, illegal, or repeated parameters and a blank, malformed, tampered, expired-format, cross-family, or parameter-mismatching cursor are `422 validation_error` — nothing is silently defaulted or normalized. The query (including its error paths) writes no resource and no audit row.
