...
Revocations record that an existing attestation has been withdrawn by an existing actor. A revocation never modifies or deletes the attestation: it is a separate immutable, append-only record with no update or delete path. No signature material is stored or echoed by revocation records.

- `POST /v1/attestation-revocations` — body contains exactly `attestation_id` (an existing attestation), `revoker_actor_id` (an existing actor), and a non-empty `reason`. The first creation returns `201` with the stable `rev_` id, the three association fields, and a UTC `created_at`; a repeat submission of the same three fields returns `200` with the existing record and writes no row or audit event, while any different combination forms an independent record. An unknown attestation is `404 attestation_not_found` and an unknown revoker is `404 unknown_actor`; blank fields, undeclared fields, and malformed JSON are `422 validation_error` and write nothing.
- `GET /v1/attestation-revocations/{revocation_id}` — full public fields, or `404 attestation_revocation_not_found`.
- `GET /v1/attestations/{attestation_id}/revocations` — that attestation's revocation records in stable creation order as `{"items", "count"}`; an unknown attestation id is `404 attestation_not_found`.

The first revocation write and its `attestation.revoked` audit event commit in a single transaction.
