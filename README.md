...
An actor can rotate the public key that authenticates it on the protected routes without creating a new attestation. Both routes authenticate every request with the same `X-PA`/`X-PT`/`X-PS` signed-header contract as the access-grant routes, and every authentication or validation failure on them is `422 validation_error` with no write.

- `POST /v1/authentication-key-rotations` — the JSON body contains exactly `actor_id` (non-empty) and `new_public_key` (standard Base64 decoding to exactly **32 bytes**); any undeclared field is rejected. The authenticated caller must be the subject `actor_id`. The first creation returns `201` with exactly `{id, actor_id, public_key, active, created_at, retired_at}` — a stable `akr_` id, the Base64 public key, `active: true`, a UTC `created_at`, and `retired_at: null`. The rotation row and its `authentication_key.rotated` audit event commit in a single transaction. A retried submission for the same actor and public key returns `200` with the original record and writes no row or audit event (even if that record has since been retired); a different public key forms an independent record. An active record's key immediately joins the actor's authentication set for the protected routes — alongside, never replacing, the keys of the actor's non-revoked attestations — so it authenticates without a new attestation, and existing non-revoked attestation keys remain valid.
- `POST /v1/authentication-key-rotations/{rotation_id}/retire` — the body is empty (the signed `body_sha256` is the digest of zero bytes). Only the subject actor may retire, and only an active record. Success returns `200` with `active: false` and a UTC `retired_at`; the record update and its `authentication_key.retired` audit event commit in a single transaction, and the retired key stops authenticating immediately while the actor's other current keys are unaffected. The record itself is preserved — there is no delete path.

An unknown subject or rotation record, a caller who is not the subject, and a repeated retirement are all `422 validation_error` and write neither a row nor an audit event. Private keys and raw signatures are never accepted, stored, or echoed — only the 32-byte public key is persisted.
