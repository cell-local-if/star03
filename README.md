...
- `GET /v1/contents/{content_id}/lineage?direction=ancestors|descendants&max_depth=...` — read-only multi-hop traversal. `direction` is required and must be exactly `ancestors` (content to direct source, following edges onward) or `descendants` (the reverse); `max_depth` is an optional integer from 1 to 32 defaulting to 8. Missing/blank/illegal `direction`, and an empty, non-integer, out-of-range, or repeated `max_depth` are `422 validation_error` — nothing is defaulted or silently normalized. The origin is never included; only contents reachable within the depth limit are returned, each deduplicated at its shortest depth, ordered by depth ascending and within a level by the discovering edge's stable creation order, as `{"items", "count", "next_cursor"}` where each item is the full public content view plus an integer `depth` and `next_cursor` is null on the final page. No reachable contents yields an empty set. An unknown origin is `404 content_not_found`. The query reads only contents and relations (no resource or audit writes), deduplicates visited nodes, and terminates even if anomalous history contains a cycle.

  Filtering and pagination extend (never alter) that traversal:

  - `relation_type` — optional, exactly `version_of` or `derived_from`; omitted, no type filtering is applied. Only contents whose **first-discovery edge** has the given type are returned. Edges of every type are still walked, so reachability and shortest-depth computation are unchanged: a node reached at depth 1 through a non-matching edge is excluded even if another matching path reaches it more deeply.
  - `min_depth` — optional integer from 1 to 32, defaulting to 1; it must not be greater than the effective `max_depth` (including the default of 8), otherwise `422 validation_error`. It floors the returned depths only; shallower edges are still traversed.
  - `limit` — optional integer from 1 to 100, defaulting to 50.
  - `cursor` — optional opaque server cursor returned as `next_cursor`; resuming requires the same `direction`, `max_depth`, `min_depth`, `relation_type`, and `limit` (and the same origin). Pages never repeat or omit items and remain ordered by depth then discovering-edge creation order. `count` is the total number of items **after filtering**, identical on every page. Cursors are stateless, deterministic, and HMAC-SHA256 signed (secret configurable via the `PROVENANCE_LINEAGE_CURSOR_SECRET` environment variable or `Settings.lineage_cursor_secret`; cursors signed under a previous secret are invalid after rotation). An empty, malformed/tampered, obsolete-format, or parameter-mismatching cursor is `422 validation_error`.
  - Any illegal value, repeated occurrence, or incompatible combination (`min_depth` > `max_depth`) of these parameters is `422 validation_error` and writes neither resources nor audit events.
