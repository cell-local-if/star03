...
def list_revocation_impact_imports(
    ...
    non-negative integer match. The three combine as logical AND; an absent
    filter imposes no restriction. Filter values are never resolved for
    existence, so an unknown value is an empty collection rather than a
    missing-resource error. Results follow the receipts' stable creation
