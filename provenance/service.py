...
def list_revocation_impact_import_reconciliations(
    ...
    The reconciliation collection is unfiltered at the storage layer: each
    receipt's current local checkpoint reconciliation is computed by the
    caller against the complete, unfiltered local revocation-impact set,
    and the optional ``local_available``/``matches`` filters apply to those
    read-time results. Results follow the receipts' stable creation order
