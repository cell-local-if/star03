...
def _supersession_identity_select(payload: ClaimSupersessionCreate):
    return select(ClaimSupersession).where(
        ClaimSupersession.superseded_claim_id == payload.superseded_claim_id,
        ClaimSupersession.replacement_claim_id == payload.replacement_claim_id,
        ClaimSupersession.reason == payload.reason,
    )


def _supersession_would_create_cycle(
    session: Session, superseded_claim_id: str, replacement_claim_id: str
) -> bool:
    """True if an edge ``superseded -> replacement`` would close a cycle.

    Edges point from an older claim to the newer claim that replaces it, so
    a cycle forms exactly when the superseded claim is already reachable
    from the replacement claim by following existing supersession edges.
    """
    visited: set[str] = set()
    frontier = [replacement_claim_id]
    while frontier:
        current = frontier.pop()
        if current == superseded_claim_id:
            return True
        if current in visited:
            continue
        visited.add(current)
        replacements = session.execute(
            select(ClaimSupersession.replacement_claim_id).where(
                ClaimSupersession.superseded_claim_id == current
            )
        ).scalars().all()
...
def create_claim_supersession(
    ...
    Both endpoints must be existing claims that assert the same content;
    the two ids must differ, and the new edge must not close a cycle. A
    self-supersession is rejected before any existence lookup; two claims
    on different content are a validation error. Unknown claims are the
    existing ``claim_not_found`` missing-resource error.

    A repeat submission of the same superseded claim, replacement claim,
    and (trimmed) reason returns the existing record with ``created=False``
    and writes no row or audit event. A different reason forms an
    independent record. On first creation the row and its
    ``claim.superseded`` audit event commit in a single transaction.
    """
    ...
    # A correction can only relate claims about the same content.
    if superseded.content_id != replacement.content_id:
        raise ClaimSupersessionValidationError("claims_cross_content")
    ...
    if _supersession_would_create_cycle(
        session, payload.superseded_claim_id, payload.replacement_claim_id
    ):
    ...
    session.add(
        AuditEvent(
            event_type=EVENT_CLAIM_SUPERSEDED, resource_id=record.id
        )
    )
    try:
        session.commit()
...
def get_claim_supersession(
    ...
    """Return a supersession by id or raise the 404 domain error."""
    record = session.execute(
        select(ClaimSupersession).where(ClaimSupersession.id == supersession_id)
    ).scalar_one_or_none()
    if record is None:
...
def list_supersessions_for_claim(
    ...
    """Return supersession records with the claim at either end, creation order.
