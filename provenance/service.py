...
# Global content-relation search paging bounds.
...
def list_content_relations_page(
    ...
    ``relation_id``, ``content_id``, ``parent_content_id``, and
    ``relation_type`` are non-empty, case- and whitespace-sensitive exact
    ...
    existence, so an unknown id is an empty result rather than a missing
    resource.
    ...
    it writes no relation, resource, or audit event.
    """
    filters = []
    ...
    total = session.execute(
        select(func.count()).select_from(ContentRelation).where(*filters)
    ).scalar_one()
