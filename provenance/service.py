...
def list_claim_supersessions_page(
    ...
    matches; all filters combine as logical AND and ``None`` means
    unfiltered. ``from_dt``/``to_dt`` are timezone-aware UTC instants
    applied as inclusive ``created_at`` bounds. Filter values are never
    resolved for existence, so an unknown id or reason is an empty result
    ...
    it writes no supersession, claim, or audit event.
    """
    filters = []
