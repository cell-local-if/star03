...
async def list_content_relations_global(
    ...
    # (covering every page) and the page is a SQL LIMIT/OFFSET window ordered
    # in SQL by created_at then the monotonic insertion sequence, so
    # ordering and paging survive a restart. Filter values are never
    # resolved for existence, so an unknown relation id or content id is an
    # empty collection rather than a 404. Each item is exactly the relation
    # public view: no internal sequence number is ever echoed.
    ...
    result = ContentRelationPageResponse(
        items=[ContentRelationResponse.model_validate(item) for item in page],
        count=total,
        next_cursor=next_cursor,
    )
    # Compact UTF-8 JSON, null literal, integral numbers only, terminated by
