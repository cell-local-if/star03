...
from provenance.errors import (
    ...
    RevocationImpactImportValidationError,
)
...
from provenance.schemas import (
    ...
    RevocationImpactImportReconResponse,
...
async def import_revocation_impacts(
    ...
    # consistency, and the claimed impact count have all passed request
    ...
    if computed_digest_hex != payload.checkpoint.impacts_digest_hex:
        raise RevocationImpactImportValidationError(
            "impacts_digest_mismatch",
            details={"computed_digest_hex": computed_digest_hex},
        )

    # Verification is decided entirely by the request body: the described
    # impacts are never resolved against local revocation state, so whether
    # they exist locally cannot change the receipt, and no revocation is
    # created or modified.
...
def _local_revocation_impact_checkpoint(
    ...
    Exactly the existing ``GET /v1/revocation-impact-package`` rules with
    no filters: every local impact is read once in the revocations' stable
    ...
    # an external verifier from the exported package alone.
    impact_payloads = [
        _revocation_impact_item(impact).model_dump(mode="json")
...
@router.get(
    ...
    response_model=RevocationImpactImportReconResponse,
)
def reconcile_impact_import(
    import_id: str, request: Request, session: DbSession
) -> RevocationImpactImportReconResponse:
    # Same boundary as the other import read routes: any (or repeated)
    # query parameter is a 422 before the receipt lookup.
    ...
    # or audit event. An unknown receipt id is the explicit
    # impact_import_not_found 404.
    record = service.get_revocation_impact_import(session, import_id)
    ...
    return RevocationImpactImportReconResponse(
        import_id=record.id,
