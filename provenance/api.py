...
from provenance.errors import QueryValidationError
...
def _query_error(field: str, msg: str, error_type: str):
...
def _parse_nonempty_filter(raw, field: str) -> str | None:
    """Parse an optional, non-empty exact-match query filter.

    Absent means unfiltered (None). A present-but-blank (whitespace-only)
    value is a client validation error, never silently treated as absent or
    matched against empty storage values.
    """
...
def list_content_evidence_bundles(
    ...
    # Read-only listing of the evidence bundles already associated with the
    # content's own claims. It never traverses lineage (so evidence on merely
    # reachable contents is excluded) and never returns raw bytes: each item
    # is the existing bundle's public fields only.
    #
    # Raw multi-values are inspected deliberately: FastAPI otherwise keeps
    # only the last value of a repeated scalar parameter and Pydantic coerces
    # numeric strings, both of which must be rejected rather than silently
    # defaulted or normalized.
    raw = request.query_params

    evidence_filter = _parse_nonempty_filter(raw, "evidence_type")
    media_filter = _parse_nonempty_filter(raw, "media_type")
    page_limit = _parse_int_param(
        raw,
        "limit",
    ...
    token = _parse_once(raw, "cursor")
    offset = 0
    if token is not None:
        try:
            claims = pagination.decode_cursor(
                request.app.state.evidence_bundle_cursor_secret,
                token,
        ...
        # The cursor only resumes the query that issued it: the path content
        # and every effective filter/limit (including their defaults) must
        # match exactly. A mismatch is a client validation error, not a new
        # query, so pages can neither skip nor repeat items.
        expected = {
            "content_id": content_id,
            "evidence_type": evidence_filter,
    ...
    # An unknown content id remains a missing resource (404), distinct from
    # both an empty collection and a malformed query. The lookup follows the
    # bundle -> claim association only, never lineage edges.
    rows = service.list_evidence_bundles_for_content(
        session, content_id, evidence_filter, media_filter
    )
    total = len(rows)
