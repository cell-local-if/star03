...
async def import_audit_exchange(
    ...
    # Structure, the fixed checkpoint/version algorithms, the claimed event
    # count, the strict event fields and timestamps, the signature metadata
    # fields, standard Base64 key/signature lengths, and the
    # 64-lowercase-hex digest spellings have all passed request validation.
    # The digest bindings are enforced here over the raw received JSON, so
    # root/array order and datetime spellings participate exactly as on the
    # stateless verification route; every mismatch is a 422
    # validation_error and writes nothing.
    body = await request.json()
    raw_package = body["package"]
    metadata = payload.signature_metadata

    # 1. The events array digest under the existing checkpoint rules.
    ...
    # 2. The full package digest: root members (checkpoint, events) and
    #    arrays keep their received order; nested object keys sort by
    #    Unicode code point under the existing package canonical rules.
    computed_package_digest_hex = canonical.audit_exchange_package_digest_hex(
        raw_package
    )
    if computed_package_digest_hex != metadata.package_digest_hex:
        raise AuditExchangeImportValidationError(
    ...
    if not ed25519.verify(metadata.public_key, message, metadata.signature):
        raise AuditSignatureInvalidError(
            details={"reason": "signature_verification_failed"}
        )
    ...
    # Verification is decided entirely by the request body: no event or
    # resource id named by the package is ever resolved against local
    # state, so whether the described events exist locally cannot change
    # the receipt, and no local resource is queried or created.
...
_AUDIT_EXCHANGE_IMPORTS_PARAMS = frozenset(
    {
        "signature_version",
        "signer_subject",
        "public_key",
...
@router.get("/audit-exchanges")
async def list_audit_exchange_imports(
    request: Request, session: DbSession
) -> Response:
    # The GET request body must be empty: carrying any bytes (even
    # whitespace, arbitrary bytes, or malformed JSON) is a 422 validated
    ...
    unknown = set(raw) - _AUDIT_EXCHANGE_IMPORTS_PARAMS
    if unknown:
        # A typo never silently changes the search.
        field = sorted(unknown)[0]
        raise _query_validation_error(
