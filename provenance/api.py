...
async def list_attestation_access_grant_revocations_global(
    ...
    # read. No authentication header is required or inspected: the search is
    # public and strictly read-only. Raw multi-values are inspected
    ...
    if unknown:
        # A typo (e.g. ``grantee_actor_id``) never silently changes the
        # search.
        field = sorted(unknown)[0]
        raise _query_validation_error(
            field, f"unknown query parameter: {field}", "value_error.unknown"
    ...
    # empty collection rather than a 404; records are matched on their own
    # columns, never via a grant lookup. Each item is exactly the single-
    # revocation public view: no raw signature, private key, payload,
    # content, evidence byte, authentication header, or ordering column is
    # ever echoed.
