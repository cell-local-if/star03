...
async def list_claim_supersessions_global(
    request: Request, session: DbSession
) -> Response:
    ...
    replacement_claim_id = _parse_nonempty_filter(raw, "replacement_claim_id")
    ...
    # ordered in SQL by created_at then the monotonic insertion sequence,
    # so ordering and paging survive a restart. Filter values are never
    # resolved for existence, so an unknown supersession id, claim id, or
    # reason is an empty collection rather than a 404. Each item is exactly
    # the supersession public view: stable id, both endpoint claim ids, the
    # reason, and its UTC created_at — never a claim payload, signature,
    # content, or evidence byte.
    ...
    result = ClaimSupersessionPageResponse(
        items=[_claim_supersession_response(item) for item in page],
        count=total,
        next_cursor=next_cursor,
    )
    # Compact UTF-8 JSON, null literal, integral numbers only, terminated by
