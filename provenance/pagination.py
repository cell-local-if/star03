...
#: Marker for cursors that page through the trust-policy search.
...
def _validate_trust_policies_claims(claims: dict[str, Any]) -> None:
    # The exact-match subject filter is either absent (null) or a non-empty
    # string; matching is case- and whitespace-sensitive, so the raw value is
    # bound.
    _optional_nonempty_str(claims, "actor_id")
    for field in ("limit", "offset"):
        if not _is_int(claims[field]):
