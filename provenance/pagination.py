...
#: Marker for cursors that page through the source-actor search.
...
def _validate_actors_claims(claims: dict[str, Any]) -> None:
    # The exact-match filters are either absent (null, meaning unfiltered)
    # or non-empty strings; matching is case- and whitespace-sensitive, so
    # the raw values are bound.
    _optional_nonempty_str(claims, "id")
    _optional_nonempty_str(claims, "name")
    _optional_nonempty_str(claims, "type")
    for field in ("limit", "offset"):
        if not _is_int(claims[field]):
            raise InvalidCursorError(f"cursor {field} must be an integer")
    if not (1 <= claims["limit"] <= 100):
...
ACTORS_CURSOR = CursorKind(
    ...
    claim_fields=("id", "name", "type", "limit", "offset"),
