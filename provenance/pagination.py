...
def _validate_checkpoint_imports_claims(claims: dict[str, Any]) -> None:
    ...
    # The event-count filter is absent (null) or a non-negative integer.
    event_count = claims["event_count"]
    if event_count is not None:
        if not _is_int(event_count):
            raise InvalidCursorError("cursor event_count must be an integer")
        if event_count < 0:
            raise InvalidCursorError("cursor event_count is out of range")
    for field in ("limit", "offset"):
        if not _is_int(claims[field]):
            raise InvalidCursorError(f"cursor {field} must be an integer")
    if not (1 <= claims["limit"] <= 100):
