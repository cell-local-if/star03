...
#: Marker for cursors that page through exchange-import reconciliation views.
...
def _validate_exchange_import_reconciliations_claims(claims: dict[str, Any]) -> None:
    # The reconciliation search carries no filters: only the effective
    # limit and the position state participate in the cursor.
    for field in ("limit", "offset"):
        if not _is_int(claims[field]):
            raise InvalidCursorError(f"cursor {field} must be an integer")
    if not (1 <= claims["limit"] <= 100):
