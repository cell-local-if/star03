...
#: Marker for cursors that page through the reviewer content retrieval.
...
def _validate_contents_claims(claims: dict[str, Any]) -> None:
    # The exact-match filters are either absent (null, meaning unfiltered) or
    # non-empty strings; matching is case- and whitespace-sensitive, so the
    # raw value is bound. The digest filter is absent (null) or the strict 64
    # lowercase hex spelling; an uppercase or trimmed spelling is a different
    # value and is never normalized.
    _optional_nonempty_str(claims, "actor_id")
