...
#: Marker for cursors that page through one subject's authentication-key
#: rotation records.
...
def _validate_authentication_key_rotations_claims(claims: dict[str, Any]) -> None:
    # The collection is scoped to one existing subject; the cursor binds the
    # exact actor id, which must be a non-empty string.
