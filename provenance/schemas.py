...
class ContentCoverageSearchItem(ContentResponse):
    """A content public view with its existing evidence-coverage summary.

    Exactly the full public content view plus the four coverage counts and
    the coverage status of the per-content coverage read -- no other field.
    """
...
class ContentCoverageSearchPageResponse(BaseModel):
    ...
    #: Total number of matching contents after filtering, independent of
    #: pagination.
    count: int
    #: Opaque server cursor for the next page, or null on the final page.
    next_cursor: str | None
