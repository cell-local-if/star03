...
#: Fixed signature-exchange version for signed audit checkpoint exchange
#: packages. The same literal is the first element of the signed JSON array,
#: so the metadata field and the signature domain-separation prefix can
#: never diverge.
...
class AuditExchangeImportCreate(BaseModel):
    """A controlled signed audit checkpoint exchange-import registration request.

    Exactly two members: ``package`` is an audit checkpoint package
    validated under the existing stateless verification structure (exactly
    ``{"checkpoint", "events"}`` with the pinned checkpoint version/
    algorithm, the claimed event count, strict event fields, and the
    canonical events digest), and ``signature_metadata`` carries the six
    signature fields. Verification is pure with respect to local state:
    no referenced id is ever resolved against a local resource.
    """
    ...
    package: AuditCheckpointVerificationCreate
    signature_metadata: AuditExchangeSignatureMetadata


class AuditExchangeImportResponse(BaseModel):
    """The public receipt for one signed audit checkpoint exchange import.

    Carries the stable ``acx_`` id, the signature version, the signing
    subject, the Base64 public key, the two digests -- the verified
    package digest and the SHA-256 digest of the verified signature --
    and the UTC ``received_at``. The package, the raw signature, and any
    private key are never persisted and so are never echoed.
