...
class ContentPageResponse(BaseModel):
    """A cursor-paginated page of content public views."""

    items: list[ContentResponse]
    #: Total number of contents after filtering, independent of pagination.
    count: int
    #: Opaque server cursor for the next page, or null on the final page.
...
#: Fixed checkpoint format version emitted for the immutable-declaration
#: correction (claim supersession) checkpoint package.
...
class ClaimSupersessionCorrectionCheckpointResponse(BaseModel):
    ...
    array. The checkpoint is derived purely from the existing
    supersession records: it introduces no resource, record, or audit
    event, and its value is stable for unchanged persisted state --
    including the empty snapshot.
    """
...
class ClaimSupersessionCorrectionPackageResponse(BaseModel):
    ...
    supersessions' stable creation order, the existing correction public
    views matched by that same filter (each exactly the existing
    five-field supersession public view). The checkpoint digest is
    computed over exactly the ``corrections`` array returned in this
    ...
    unescaped non-ASCII, UTF-8); both derive from a single read-only
    state read, so they can never disagree. The package introduces no
    resource, record, or audit event; an empty match yields
    ``"corrections": []`` together with the digest of the empty array.
    """
...
class CspVerificationCorrection(BaseModel):
    """One correction under stateless verification: exactly the public view.

    The structure must match the exported correction public view field for
    field -- ``id``, both endpoint claim ids, ``reason``, and a strict
    RFC 3339 UTC ``created_at`` -- with no undeclared member (so no claim
    ...
    timestamp exactly as spelled on the wire. Verification never resolves
    any id against local state: fabricated but structurally consistent
    identifiers verify exactly like known ones.
    """
...
class CspVerificationCheckpoint(BaseModel):
    """The claimed checkpoint under stateless correction verification.

    Exactly the four-field checkpoint structure the export serves: the
    fixed ``checkpoint_version`` and ``digest_algorithm``, the claimed
    correction count, and the claimed corrections digest. Undeclared
...
class CspVerificationCreate(BaseModel):
    ...
    elements, each exactly the exported correction public view.
    Verification is pure: it reads and writes no server state, so no
    supersession, claim, or other resource id is ever resolved against
    local state.
    """

    model_config = ConfigDict(extra="forbid")

    checkpoint: CspVerificationCheckpoint
    corrections: list[CspVerificationCorrection]
...
class CspVerificationResponse(BaseModel):
...
class RevocationImpactImportCreate(RevocationImpactVerificationCreate):
    """A controlled import of one offline-verified revocation-impact checkpoint.
    ...
    ``checkpoint.impact_count`` elements, each exactly the exported impact
    public view and internally consistent. The digest match itself is
    ...
    resolves any revocation, attestation, actor, content, or bundle id
    against local state: the described impacts are not created, modified,
    or queried, and they need not exist locally.
    """
...
class RevocationImpactImportReconciliationItem(BaseModel):
    ...
    ``received_at``) plus ``local_available`` -- true when the current local
    revocation-impact set is non-empty (an empty set still yields a
    deterministic checkpoint) --, ``local_checkpoint`` -- the four-field
    checkpoint computed over the complete, unfiltered local revocation-impact
    set under the existing checkpoint rules --, and ``matches``, true only
    when the receipt's ``checkpoint_version``, ``impact_count``, and
    ``impacts_digest_hex`` all equal that local checkpoint. The imported
    impacts array is never echoed.
    """
...
class RevocationImpactImportReconciliationPageResponse(BaseModel):
    """A cursor-paginated page of impact-import receipt reconciliations."""
...
#: Fixed signature-exchange version for signed impact-recon packages. The
#: same literal is the first element of the signed JSON array, so the
#: metadata field and the signature domain-separation prefix can never
#: diverge.
IMPACT_RECON_EXCHANGE_SIGNATURE_VERSION = (
    "provenance-impact-recon-exchange-v1"
)
#: The sole digest algorithm a signed exchange package may name.
IMPACT_RECON_EXCHANGE_DIGEST_ALGORITHM = IMPACT_RECON_DIGEST_ALGORITHM


class ImpactReconExchangeSignatureMetadata(BaseModel):
    """The signature metadata accompanying a signed impact-recon package.

    Exactly six members: the fixed exchange ``signature_version``, the
    non-empty signing ``signer_subject``, the standard-Base64 Ed25519
    ``public_key`` (decoding to exactly 32 bytes), the standard-Base64
    ``signature`` (decoding to exactly 64 bytes), and the package digest
    ``package_digest_algorithm`` (``"sha256"``) with its 64-lowercase-hex
    ``package_digest_hex``. Undeclared members are rejected rather than
    ignored, so no private key or other raw material can enter the
    request. The raw signature is verified but never persisted.
    """
    ...
    signature: bytes
    package_digest_algorithm: Literal[IMPACT_RECON_EXCHANGE_DIGEST_ALGORITHM]
...
class ImpactReconExchangeImportCreate(BaseModel):
    """A controlled signed impact-recon exchange-import registration request.

    Exactly two members: ``package`` is an impact-recon package validated
    under the existing stateless verification structure (exactly
    ``{"checkpoint", "entries"}`` with the pinned package version/
    algorithm, the claimed entry count, strict entry fields, and the
    canonical entries digest), and ``signature_metadata`` carries the six
    signature fields. Verification is pure with respect to local state:
    no referenced id is ever resolved against a local resource.
    """
...
class ImpactReconExchangeImportResponse(BaseModel):
    """The public receipt for one signed impact-recon exchange import.

    Carries the stable ``irx_`` id, the signature version, the signing
    subject, the Base64 public key, the two digests -- the verified
    package digest and the SHA-256 digest of the verified signature --
    and the UTC ``received_at``. The package, the raw signature, and any
    private key are never persisted and so are never echoed.
    """
