...
def authentication_key_rotation_id(actor_id: str, public_key_hex: str) -> str:
    """Return a stable ``akr_``-prefixed identifier for a key rotation.

    The identity is exactly the idempotency key: the subject actor and the
    new public key. The material is a canonical JSON array so fields
    containing separators cannot collide with different field splits.
    """
    material = json.dumps(
