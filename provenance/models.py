...
class AuditExchangeImportRecord(Base):
    """An immutable receipt for one signed, offline-verified audit checkpoint
    exchange package.

    The record is the controlled cross-system counterpart of the stateless
    audit-checkpoint verification route: a package whose structure, event
    count, and array digest all pass the existing verification rules, and
    whose ``signature_metadata`` carries an Ed25519 signature verified over
    the canonical array binding the fixed exchange version, the signing
    subject, the package digest algorithm, and the package digest, is
    registered exactly once. The receipt stores only the package identity
    and the accepted signature's claims -- the signature version, the
    signing subject, the signer's 32-byte public key, both digest
    ...
    full package and signature must be re-presented on a retry. The
    signature is checked but its raw bytes are never persisted.
    Verification never depends on whether the referenced events exist
    locally (they are never queried, created, or modified), so no foreign
    keys and no local resource lookup participate in the decision.

    Records are append-only and immutable; there is deliberately no update
    or delete path. The package identity triple is unique, so a retried
    submission for the same package with the same signing subject, key, and
    signature returns the original record without another audit event,
    while a different subject, key, or signature for the same package is
    refused with the original record untouched.
    """
