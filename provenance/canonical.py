...
def claim_supersession_corrections_digest_hex(corrections: Any) -> str:
    """Return the SHA-256 hex digest of the canonical corrections array.
    ...
    emitted unescaped, and UTF-8 encoding -- the same canonical form as
    :func:`audit_events_digest_hex`, so the digest is reproducible offline
    from the exported ``corrections`` member alone, including an empty
    array (the digest of ``[]``).
    """
...
def canonical_audit_checkpoint_package_bytes(package: Any) -> bytes:
    ...
    ``events``) and the events array keeps its element order, while every
    nested object's members are sorted by Unicode code point, with compact
...
def audit_checkpoint_package_digest_hex(package: Any) -> str:
    """Return the SHA-256 hex digest of the canonical audit-package bytes."""
    return hashlib.sha256(
        canonical_audit_checkpoint_package_bytes(package)
    ).hexdigest()
